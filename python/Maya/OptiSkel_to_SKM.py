...
#print(scriptDir)
scriptDir = "//vuwstocoissrin1.vuw.ac.nz/SODI_RapidMedia_01/Software/OptiSkelForUEScripts/python/Maya/"
...
def addNamespaceToStrings(inputList, NS, namespaceToken):
    outlist = []
    for each in inputList:
        outlist.append("{0}{1}{2}".format(NS,namespaceToken,each))
    return outlist
...
def buildSkeletonFromSelected(namespaceToken=":"):
    # get namespace for selected
    NS = cmds.ls(sl=True)[0].split(namespaceToken)[0]
    ...
    cmds.setAttr(NS + namespaceToken + 'Hips.translateX', 0)
    cmds.setAttr(NS + namespaceToken + 'Hips.translateZ', 0)
    ...
    scalep5 = addNamespaceToStrings([u"LeftHandMiddle1_geo",u"RightHandMiddle1_geo"], NS, namespaceToken)
    scalep2 = addNamespaceToStrings([u'LIndex3End_geo', u'LeftHandIndex3_geo', u'LeftHandIndex2_geo', u'LMiddle3End_geo', 
        ...
        u'RightHandThumb1_geo',u'RightHandThumb2_geo', u'RightHandThumb3_geo', u'RThumb3End_geo'], NS, namespaceToken)
    # make a material
    spherelist = []
    sticklist = []
        ...
        if c:
            for each in c:
                ...
                noSpheresList = addNamespaceToStrings(["RightUpLeg", "LeftUpLeg","RightHandIndex1","RightHandThumb1","RightHandRing1","RightHandPinky1", 
                    "LeftHandIndex1","LeftHandThumb1","LeftHandRing1","LeftHandPinky1", "LeftShoulder", "RightShoulder"],NS, namespaceToken)
                if each in noSpheresList:
                    print("{0} found in noSpheresList".format(each))
...
""" Use it like this in the script editor ####
import sys
import maya.cmds as cmds
...
if scriptDir not in sys.path:
    sys.path.append(scriptDir)
# select the top level of the imported fbx motion from optitrack, set the namespace specifier (: or _)
import OptiSkel_to_SKM as osk    
osk.buildSkeletonFromSelected(namespaceToken=":")
"""